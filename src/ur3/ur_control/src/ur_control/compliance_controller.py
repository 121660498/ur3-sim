...
class CompliantController(Arm):
    ...
    def set_hybrid_control_trajectory(self, trajectory, max_force_torque, timeout=5.0,
        ...
        while not rospy.is_shutdown() \
            ...
            # Clamp velocity output to prevent excessive speeds
            dxf[:3] = np.clip(dxf[:3], -0.1, 0.1)  # linear velocity: ±0.1 m/s
            dxf[3:] = np.clip(dxf[3:], -1.0, 1.0)  # angular velocity: ±1.0 rad/s
